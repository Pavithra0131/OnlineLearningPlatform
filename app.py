...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:''@localhost/onlinelearn'
...
app.config['MAIL_USERNAME'] = 'fake.ac.4.test@gmail.com'  # Your email address
app.config['MAIL_PASSWORD'] = 'qpla noqn onps kagt'  # Your email password
    ...
    if request.method == 'POST':
        ...
        if teacher:
            if teacher.password == password:
                ...
                msg = Message('Your OTP for Sign In', sender='fake.ac.4.test@gmail.com', recipients=[email])
...
def teacher_dashboard():
    ...
    if teacher_id:
        ...
        if teacher:
            ...
            return render_template("teacher_dashboard.html", courses=all_data, registered_students=registered_students,teacher_name=teacher.name)
''''
@app.route('/teacher_dashboard')
def teacher_dashboard():
    all_data = Course.query.all()
